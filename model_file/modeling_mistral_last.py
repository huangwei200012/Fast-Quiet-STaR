...
class MistralForCausalLM(MistralPreTrainedModel):
    ...
    def forward(
        ...
        # if self.training_steps // self.gradient_accumulation_steps == 101:
        #     self.rm_initialized = False
        #     print("设置 self.rm_initialized 为False")
        ...
        if not self.rm_initialized and (self.n_ahead > 1 or not self.base_original_mode):
            ...
            print("设置self.rm_initialized为True,同时初始化head权重")
        ...
        # print("fwd_iters:",fwd_iters)
        # print("self.n_ahead:",self.n_ahead)
        # print("self.n_ahead_talk:",self.n_ahead_talk)
        ...
        for ahead_idx in range(fwd_iters):
            ...
            if ahead_idx == 0:
                ...
                base_logits_softmax = F.softmax(base_logits,dim=-1).detach()
                base_logits_max_index = torch.argmax(base_logits_softmax,dim=-1)
                selected_values = torch.gather(base_logits_softmax, 2, base_logits_max_index.unsqueeze(2))
                selected_values = selected_values.squeeze(2)
                queit_mask = selected_values <= 0.9
                norm_mask = selected_values > 0.9
                # 在这里根据selected_values去生成了一个mask
                mask_expanded = queit_mask.unsqueeze(2).expand_as(base_logits)

                # 在这里加一个计算logits熵的过程，然后做成一个mask，然后修改Start sought的Token，
                # 把Mask操作引入进来？
            else:
                ...
                #     logits = torch.where(mask_expanded, logits, base_logits)
            shift_idx = 1
            shift_logits = logits[..., :-shift_idx, :].contiguous()
            ...
            # shift_base_logits = torch.where(shift_base_logits == self.tokenizer.pad_token_id, -100, shift_base_logits)
            # shift_logits = torch.where(shift_logits == self.tokenizer.pad_token_id, -100, shift_logits)
            ...
            if self.training:
                # 下面是第一版损失暂时叫actor_loss
                shift_base_logits = labels_kl[..., :-shift_idx, :].contiguous()
                shift_base_logits = shift_base_logits.view(-1, self.config.vocab_size)
                shift_base_logits = shift_base_logits.to(shift_logits.device)
                base_unreduced_loss = loss_fct(shift_base_logits, shift_labels)
                unreduced_loss = loss_fct(shift_logits, shift_labels)
                unreduced_loss = unreduced_loss.reshape(logits.shape[0], -1)
                base_unreduced_loss = base_unreduced_loss.reshape(logits.shape[0], -1)
                original_dqn_reward = base_unreduced_loss.detach() - unreduced_loss
                probabilities_2d = logits.view(-1, logits.size(-1))
                prev_probabilities_2d = F.gumbel_softmax(probabilities_2d, tau=1, hard=True, dim=-1)
                nonzero_indices = prev_probabilities_2d.nonzero()
                action_loglikelihoods = F.log_softmax(probabilities_2d / self.reinforce_temperature, dim=-1)[nonzero_indices[:, 0], nonzero_indices[:, 1]]
                action_loglikelihoods_2d = action_loglikelihoods.reshape(batch_size, -1)[:, :-1]
                # discard rewards below the mean
                fixed_policy_reward = original_dqn_reward.detach().clamp(min=0)
                actor_loss = -fixed_policy_reward * action_loglikelihoods_2d[:, :fixed_policy_reward.shape[-1]].to(fixed_policy_reward.device)
                loss_rm = actor_loss.mean()
                # 下面是在actor_loss上进行了一个小改进，暂时叫reward_loss
                # shift_base_logits = labels_kl[..., :-shift_idx, :].contiguous()
                # shift_base_logits = shift_base_logits.view(-1, self.config.vocab_size)
                # shift_base_logits = shift_base_logits.to(shift_logits.device)
                # base_unreduced_loss = loss_fct(shift_base_logits, shift_labels)
                # original_dqn_reward = unreduced_loss - base_unreduced_loss.detach()
                # original_dqn_reward = original_dqn_reward.clamp(min=0)
                # loss_rm = original_dqn_reward.mean()
                # 下面是mseloss
                # 下面是weight mse loss
        loss = self.loss_mean(unreduced_loss)
        loss = loss * self.base_loss_beta
        if self.training:
            loss = loss_rm * self.policy_loss_beta
